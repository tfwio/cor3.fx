...
namespace Cor3
{
	public class font_util
	{
		...
		static public bool IsFitted(Font fnt)
		{
			...
			foreach (FloatPoint hp in MeasureRange(str,fnt))
				if ((int)hp.X != x)
				{
...
//			Global.cstat(ConsoleColor.Green,true);
		...
		static public FloatPoint Measure(string str, Font fnt) { return TextRenderer.MeasureText(str,fnt); }
		static public FloatPoint Measure(string str, Font fnt,Graphics fx) { return fx.MeasureString(str,fnt); }
		static public FloatPoint Measure(Font fnt) { return TextRenderer.MeasureText(fnt.Name,fnt); }
		static public FloatPoint Measure(Font fnt, Graphics fx) { return fx.MeasureString(fnt.Name,fnt); }
		static public FloatPoint Measure(int index) {
			if (index==-1) return FloatPoint.Empty; FloatPoint hp = FloatPoint.Empty;
