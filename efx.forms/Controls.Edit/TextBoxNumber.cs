...
namespace efx.Forms.Controls
{
	...
	{
		public TextBoxNumber() : base(){Value = _default;}
		public TextBoxNumber(float val) : base(){Value = val;}
		#region automation
		...
		public void NumberC(float val){if(NumberChange!=null) NumberChange(val);}
		[System.ComponentModel.BrowsableAttribute(false)]
		public float? Value
		{
			get {try { return _value; } catch { Text = ((float)_value).ToString("##0.0");} return Value;}
			set { _value=value; Text = ((float)_value).ToString("##0.0"); NumberC((float)_value); }
		}
